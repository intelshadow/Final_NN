...
public function addQuestion_completar($enunciado, $respuesta1, $dificultad, $lenguaje, $tema)
    {
        try {
            // Insertar enunciado y obtener su ID
            $stmt = $this->con->prepare("INSERT INTO enunciados_completar (texto_enunciado_completar, lenguaje_completar, dificultad_completar, tema_completar) VALUES (?, ?, ?, ?)");
            $stmt->bind_param("ssis", $enunciado, $lenguaje, $dificultad, $tema);
            $stmt->execute();
            $enunciadoId = $this->con->insert_id;
    ...
    public function addQuestion_escribir($enunciado, $respuesta1, $dificultad, $lenguaje, $tema)
    {
        try {
            // Insertar enunciado y obtener su ID
            $stmt = $this->con->prepare("INSERT INTO enunciados_escribir (texto_enunciado_escribir, lenguaje_escribir, dificultad_escribir, tema_escribir) VALUES (?, ?, ?, ?)");
            $stmt->bind_param("ssis", $enunciado, $lenguaje, $dificultad, $tema);
            $stmt->execute();
            $enunciadoId = $this->con->insert_id;
...
public function plantillacompletar($lenguaje, $tema) {
    ...
    try {
        // Realiza la consulta SQL con marcadores de posición
        $sql = "SELECT ec.texto_enunciado_completar AS enunciado,
                rc.texto_pregunta AS respuesta_correcta
            FROM enunciados_completar ec
            JOIN respuestas_completar rc ON ec.id_enunciado_completar = rc.enunciado_id
...
public function plantillaescribir($lenguaje, $tema) {
    ...
    try {
        // Realiza la consulta SQL con marcadores de posición
        $sql = "SELECT ee.texto_enunciado_escribir AS enunciado,
                re.texto_pregunta AS respuesta_correcta
            FROM enunciados_escribir ee
            JOIN respuestas_escribir re ON ee.id_enunciado_escribir = re.enunciado_id
